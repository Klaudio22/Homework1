...
function majorOfEachStudent(){
    ...
    for(let student of students)
    {
        for(let course of student.courses)
        {
            ...
            {
                if(!coursesOfStudents[student.name])
                { 
                    coursesOfStudents[student.name] = []; 
                }
                if(course == crs.section)
                { 
                    coursesOfStudents[student.name].push(course.substring(0,3));
                }
            }
        }
    }

    for(let x in coursesOfStudents)
    {
        counts = [];
        coursesOfStudents[x].forEach(function (x) { counts[x] = (counts[x] || 0) + 1; });
       
        const max=Math.max(...Object.values(counts));
        const numberOfSameDepartment = Object.fromEntries(Object.entries(counts).filter(([name, num]) => num == max));
        if(max==1)
        {
            studentsMajor.push(x, ["No major"]);
        }
        else
        {
            studentsMajor.push(x, Object.keys(numberOfSameDepartment))
        }
    }
